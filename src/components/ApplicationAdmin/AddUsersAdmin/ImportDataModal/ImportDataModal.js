...
import { validateStudentData, validateTeacherData } from './../ExcelImportExport/csvParser';
import CustomModal from '../../../../services/CustomModal/CustomModal';

function ImportDataModal({ open, setOpen, importedData, userType = 'student', onConfirmImport }) {
        ...
        <div className={styles.modal}>
          <div className={styles.header}>
            <h3 className={styles.title}>
              ...
              ตรวจสอบข้อมูล{userType === 'student' ? 'นักศึกษา' : 'อาจารย์'}
            </h3>
                  ...
                  <table className={styles.previewTable}>
                    <thead>
                      <tr>
                        ...
                        <th>แถว</th>
                        <th>อีเมล</th>
                        <th>{userType === 'student' ? 'รหัสนักศึกษา' : 'รหัสอาจารย์'}</th>
                        <th>ชื่อ-นามสกุล</th>
                        <th>คณะ</th>
                        <th>{userType === 'student' ? 'สาขาวิชา' : 'ภาควิชา'}</th>
                        {userType === 'student' && <th>อาจารย์ที่ปรึกษา</th>}
                        {userType === 'student' && <th>ปีการศึกษา</th>}
                        {userType === 'teacher' && <th>คณบดี</th>}
                      </tr>
                    </thead>
                    <tbody>
                      {previewData.map((row, index) => (
                        <tr key={index} className={row.status === 'error' ? styles.errorRow : styles.validRow}>
                          ...
                          <td>{userType === 'student' ? row.Student_Code : row.Teacher_Code}</td>
                          <td>
                            {userType === 'student'
                          ...
                          <td className={styles.facultyCell}>{row.Faculty_Name}</td>
                          <td>{row.Department_Name}</td>
                          {userType === 'student' && <td>{row.Teacher_Code}</td>}
                          {userType === 'student' && <td>{row.Student_AcademicYear}</td>}
                          {userType === 'teacher' && (
                            <td>
                              {row.Teacher_IsDean === 'true' || row.Teacher_IsDean === true ? (
                                <span className={styles.deanBadge}>คณบดี</span>
                              ) : (
                                <span className={styles.normalBadge}>อาจารย์</span>
                              )}
                            </td>
                          )}
                        </tr>
                        ...
                        <div className={styles.errorHeader}>
                          ...
                          <span>
                            แถวที่ {error.rowIndex}: {
                              userType === 'student'
                                ? `${error.Student_FirstName || ''} ${error.Student_LastName || ''}`
                                : `${error.Teacher_FirstName || ''} ${error.Teacher_LastName || ''}`
                            }
                          </span>
                        </div>
