...
export const useStudents = () => {
  ...
  const fetchStudents = useCallback(async (params = {}) => {
    try {
      ...
      const studentParams = {};
      studentParams.includeGraduated = params.includeGraduated === true ? 'true' : 'false';
      ...
      const apiEndpoint = '/api/admin/students/incomplete-activities';

      const studentsRes = await axios.get(getApiUrl(apiEndpoint), {
      ...
      if (studentsRes.data && studentsRes.data.status) {
        const incompleteStudents = studentsRes.data.data;
        const transformedStudents = incompleteStudents.map(student => {
        ...
        const validStudents = transformedStudents.filter(s => s !== null);

        if (validStudents.length < transformedStudents.length) {
          console.warn(`Removed ${transformedStudents.length - validStudents.length} students without valid ID`);
        }

        validStudents.sort((a, b) => a.completedActivities - b.completedActivities);

        setStudents(validStudents);
        if (Object.keys(params).length > 0 && params.searchQuery) {
          const searchDescription = `ค้นหาข้อมูลนักศึกษาที่กิจกรรมไม่ครบ: ${JSON.stringify(params)}`;
          try {
            await logSystemAction(0, searchDescription);
          } catch (logError) {
            console.warn('Failed to log search action:', logError);
          }
        }
      } else {
      ...
      if (axios.isAxiosError(err)) {
        if (err.code === 'ECONNABORTED') {
          ...
          switch (err.response.status) {
            case 400:
              errorMessage = 'คำขอไม่ถูกต้อง กรุณาตรวจสอบข้อมูลและลองใหม่';
