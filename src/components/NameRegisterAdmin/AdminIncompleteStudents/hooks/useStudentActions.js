...
export const useStudentActions = ({
  ...
  const handleAddStudent = useCallback(async () => {
    ...
    navigate('/application/add-user', {
      state: { from: '/name-register/student-incomplete-activities' }
    });
  ...
  const handleShowStudentSummary = useCallback(async (students, filterInfo) => {
    ...
    try {
      await logSystemAction(
        0,
        `ดูสรุปข้อมูลนักศึกษาที่กิจกรรมไม่ครบ: ${students.length} คน ${filterInfo?.department ? `สาขา: ${filterInfo.department}` : ''}`
      );
    } catch (error) {
    ...
    const avgActivities = totalStudents > 0 ? (totalActivities / totalStudents).toFixed(1) : 0;

    const summaryMessage = `
    สรุปข้อมูลนักศึกษาที่กิจกรรมไม่ครบ (≤10 กิจกรรม):
    ...
    กิจกรรม:
    - จำนวนกิจกรรมเฉลี่ย: ${avgActivities} กิจกรรม/คน
...
${filterInfo?.academicYear ? `ปีการศึกษา: ${filterInfo.academicYear}` : ''}
  ...
  const handleRefreshStudents = useCallback(async () => {
    try {
      await logSystemAction(0, 'รีเฟรชข้อมูลนักศึกษาที่กิจกรรมไม่ครบ');
  ...
  const handleBulkExport = useCallback(async (students, filterInfo, options = {}) => {
    ...
    คุณต้องการส่งออกข้อมูลนักศึกษาที่กิจกรรมไม่ครบทั้งหมด ${students.length} คน เป็นไฟล์ Excel หรือไม่?

    ข้อมูลที่จะส่งออก:
    - ข้อมูลส่วนตัวนักศึกษา
    - ข้อมูลการศึกษา
    - จำนวนกิจกรรมที่สำเร็จ
    - สถานะการใช้งาน
    - สรุปตามสาขาและคณะ
      ...
      },
      {
        ...
        onClick: async () => {
          ...
          try {
            await Promise.all([
              logBulkOperation(
                'ส่งออกข้อมูลนักศึกษาที่กิจกรรมไม่ครบแบบ Bulk',
                students.length,
